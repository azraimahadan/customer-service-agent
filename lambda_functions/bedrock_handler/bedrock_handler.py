...
def lambda_handler(event, context):
    try:
        ...
        try:
            ...
            if e.response.get('Error', {}).get('Code') == 'TextLengthExceededException':
                truncated_text = agent_response[:2500]
                print(f"WARNING: Text length exceeded, retrying with truncated text")
                tts_response = polly_client.synthesize_speech(
                    Text=truncated_text,
                    OutputFormat='mp3',
                    VoiceId='Joanna'
        ...
        # Store enhanced troubleshooting response
        ...
        return {
            ...
            'body': json.dumps({
                ...
                'tools_executed': troubleshooting_data['tools_executed'],
...
def get_knowledge_base_context(query, analysis_data):
    ...
    try:
        ...
        if analysis_data.get('labels'):
            label_names = [l['Name'] for l in analysis_data['labels']]
            enhanced_query += f" {' '.join(label_names)}"
